...
def twofifty(movies, moviesList, want):
    ...
    while counter < want:
        ...
        if(currentMovie not in moviesList):
            f.write(str(title) + "|" + str(year) + "|" + str(genre) + "|" + str(rating) + "|" + str(duration) + "|" + str(description) + "|" + str(directors) + "|" + str(starsList) + "\n")
            moviesList.append(currentMovie)
    f.close()

pagesNeeded = math.ceil(count/250)
f = open("scraped_movies.csv", "w")
f.write("title|year|genre|rating|duration|description|directors|starsList\n")
f.close()
moviesList = []
